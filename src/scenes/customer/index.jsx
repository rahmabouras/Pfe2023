...
const Customer = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/api/customers')
      ...
      .catch(error => console.error(`There was an error retrieving the customers: ${error}`));
  }, []);
  ...
  const handleDelete = () => {
    axios.delete(`http://localhost:3000/api/customers/${selectedId}`)
      .then(res => {
        ...
        setCustomers(customers.filter((customer) => customer._id !== selectedId));  // removing the deleted customer
      });
    setOpen(false);
  };
  ...
  return (
    <Box m="20px">
      <Header title="CUSTOMERS" subtitle="Managing of Customer" />
      <Box
        m="40px 0 0 0"
        height="75vh"
        sx={{
