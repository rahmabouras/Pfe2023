...
const EditContact = () => {
  ...
  const handleFormSubmit = (values, { setSubmitting }) => { 
    axios.put(`http://localhost:5000/api/contacts/${id}`, values) // <-- call the PUT API
      .then(response => {
        console.log(response.data);
        setSubmitting(false); // stop showing the submitting state in form
        navigate("/contacts"); // navigate to /customer
      })
      .catch(error => {
  ...
  return (
    <Box m="20px">
      <Header title="EDIT CONTACT" subtitle="Update Existing Contact" />
